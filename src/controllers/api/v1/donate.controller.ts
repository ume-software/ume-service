...
export class DonateController extends BaseController {
    ...
    customRouting() {
        ...
        this.router.post(
            ...
            this.route(this.donateForProvider)
        );
    }
    @ApiOperationGet({
        path: "/provider/top",
    ...
    async topDonateProvider(req: Request, res: Response) {
        ...
        const result = await this.service.topDonateProvider(
            duration as ETopDonateDuration,
    ...
    async donateForProvider(req: Request, res: Response) {
        const donateProviderRequest = new DonateProviderRequest(req.body);
        const creatorId = req.tokenInfo?.id;
        const result = await this.service.donateForProvider(
            creatorId!,
            donateProviderRequest
        );
        this.onSuccess(res, result);
    }
}
