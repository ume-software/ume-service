...
export class CoinSettingRepository extends BasePrismaRepository {
  ...
  async getConvertCoinToMoneyForBuyCoinRate(unitCurrency: UnitCurrency, paymentSystemPlatform: PaymentSystemPlatform) {
  ...
  calculateCoinToMoneyForBuyCoin(amountCoin: number, option = {
  ...
  async convertCoinToMoneyForBuyCoin(amountCoin: number, unitCurrency: UnitCurrency, paymentSystemPlatform: PaymentSystemPlatform) {
    const option = await this.getConvertCoinToMoneyForBuyCoinRate(unitCurrency, paymentSystemPlatform)
  ...
  async calculateCoinDonateForProvider(amountCoin: number): Promise<number> {
    return await this.calculateForProvider(amountCoin, CoinSettingType.PROVIDER_GET_COIN_DONATE)
  }
