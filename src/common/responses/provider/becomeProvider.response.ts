...
@ApiModel({
    description: "User register become provider response",
})
export class RegisterBecomeProviderResponse {
    @ApiModelProperty({
    ...
    @ApiModelProperty({
        description: "Description",
        required: true,
        example: RegisterProviderRequestStatus.PENDING,
        enum: Object.values(RegisterProviderRequestStatus),
    })
    status!: RegisterProviderRequestStatus;

    @ApiModelProperty({
        description: "Admin note when handle request",
        required: true,
        example: "This is admin noted",
    })
    adminNoted!: string;
}
