...
export class ImageService {
  async upload(req: any): Promise<string[]> {
    ...
    for (const file of listFiles) {
      ...
      if (
        !(Object.keys(resize).length === 0 && resize.constructor === Object)
      ) {
        ...
        if (file.mimetype?.includes("gif")) {
          ...
          writeFileSync(newPathName, readFileSync(oldPathName));
        } else {
          await sharp(oldPathName)
            .resize({
              ...resizeOption,
            })
            .toFile(newPathName);
      ...
      const result = `${req.protocol}://${join(
        hostname,
        join("api/image", encodeURIComponent(newFilename))
      ).replace(/\\/g, "/")}`;
